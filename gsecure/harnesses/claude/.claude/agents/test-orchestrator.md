...
description: Drive ONE batch of the unit-test build (void/test/Flow-based-plans/unit-test-<batch>.md) end to end: pre-flight, worktree-per-phase waves, designer/adversary/implementer/triage loop, production-defect fix paths, merges, ledgers and commits. Run as the MAIN session via /test-build — it owns the human gates.
