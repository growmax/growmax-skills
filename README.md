...
|---|---|---|
| `gmax-setup` | `/growmax-skills:gmax-setup` | **Install the gmax agentic-development workflow into the current project** — the operating layer that makes every bug/feature follow one gated spine: understand (business context) → design (architecture) → plan (waves/phases) → build → senior review. Copies 6 personas + 7 skills + `standards/` + `workflow.config.md` + harness shims non-destructively, appends to `AGENTS.md` under a `<!-- gmax -->` marker (never replaces), DETECTS the project's existing knowledge base (e.g. `docs/product/` from `/learn-app`) and records its pointer — never creates one. Human-confirmed at every step; opt-in per project. Run once per repo, main session. → **[Usage guide](docs/gmax.md)** |
| `gmax-update` | `/growmax-skills:gmax-update` | **Refresh a project's installed gmax layer** after a plugin update — diffs `.gmax-version` against the shipped `gmax/VERSION`, lists changed canonical files, and on one confirm re-copies ONLY `agents/`, `skills/`, `harnesses/` (plus the `<!-- gmax -->` AGENTS.md block). Never touches `workflow.config.md`, `standards/`, the knowledge base, or `void/`. |
...
| `repo-sweep` | `/growmax-skills:repo-sweep [init\|run\|report\|selftest]` | **Provably read every file in a repo — a factory line for a codebase.** Instead of one agent that reads 4,778 files and degrades as its context fills, the repo is split into hundreds of small units and each gets a **brand-new agent with an empty context** that reads only those files, writes what it found, and is discarded — so unit #300 gets the same sharp context as unit #1. The two decisions a degrading agent is worst at are taken away from the model entirely: **what the work is** (`build-ledger.mjs` enumerates it from `git ls-files` and *asserts* the partition is total and disjoint — every sweepable file in exactly one unit, every skipped file listed with a reason, so `tracked = swept + excluded` always reconciles) and **whether it is done** (`verify-unit.mjs` accepts a unit only on claims you cannot make without opening the file: exact line counts, symbols that literally occur, `path:line` citations landing in-unit on non-blank lines, no templated prose). `ledger-update.mjs` is the **only** writer of status and **re-runs the checker itself** before recording a pass — an agent saying "I'm done" does not make it done. A second fresh agent that never met the writer re-reads the code and grades it. Three strikes parks a unit as `needs_human` and the line keeps moving; a crash loses only in-flight units; `selftest.mjs` feeds the checker ten artifacts (one truthful, nine lying) and asserts each lie is caught **by name**. Coverage is counted off disk — `units 3/324 passed · files 9/4778 (0.2%)` is a measurement, not an agent's opinion. Repo-agnostic: works unconfigured on any git repo, and an optional `sweep.config.json` adds the repo's own excludes, sweep order, and **invariants** (injected into both prompts, which is how a grader knows an unscoped query is a missed risk rather than a style opinion). Ships the `repo-sweep` skill + `workflows/repo-sweep.workflow.js`. Cost is real — a ~5k-file repo is hours and tens of millions of tokens. |
