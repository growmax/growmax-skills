...
| `bugfix` | `/growmax-skills:bugfix [--gated\|--auto\|--shadow] <bug report>` | **The whole bug-fix lap in ONE session, ROUTED** — a chain of `confirm-bug` → `fix-bug` that first runs a 90-second `bug-triage` pass (haiku, read-only) returning **evidence, never a decision**: symptom class, confidence, call sites, the **predicted fix paths** (what a fix would edit — not the directory the broken screen lives in), money/authz/tenant/schema flags, one in-repo precedent or none, and an honest count of competing hypotheses. A **deterministic table** — no model judgment, and the orchestrator re-verifies the precedent and paths itself — picks the route: **AUTO** (0 interrupts: precedent auto-applied, the red confirmed by parsing the runner's own reporter output, frozen at a pushed tag, then an **evidence-bundle PR that is never merged**) · **CONFIRM** (1 plain-language confirm-or-overrule question) · **GATED** (the classic three gates). **Every mechanical grader runs on every route** — red-first, the frozen pushed tag, the mutation check, the blind default-FAIL validator, the `protect-repro` hook; a machine freeze is recorded honestly as `confirmed_mode: "machine"`, never as a human approval that didn't happen. The table is **fail-closed**: money/auth/tenant/schema, product ambiguity, low confidence, competing causes, a precedent that doesn't check out, or a tag that won't push all land on GATED — **routing can only ever add gates, never remove them.** Split into the two commands (or pass `--gated`) for the air gap on maximum-rigor bugs, long diagnoses, hand-offs, or resuming. → **[Usage guide](docs/bugfix-pipeline.md)** |
