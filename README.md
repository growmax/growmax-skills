...
| `security-audit` | `/growmax-skills:security-audit [scope] [--find-only] [--only <owasp-ids>]` | **Whole-app OWASP Top 10 audit → one gate → approved-only fix batch.** Where `feature-review` reviews one branch's *diff*, this audits the *whole app*: recon the attack surface, then fan out the **same `scale-security-reviewer`** per module shard (A01–A05, A07–A10 — authz, tenant scoping, injection, validation, mass assignment, SSRF, security-event logging) plus a **Trivy shard** (`trivy fs`) for the code-reader's blind spots (A06 dependency CVEs, secrets, IaC misconfig — reusing `github-repo-analyzer`'s scan machinery and verdict thresholds). Every finding is **adversarially refuted** before it reaches you, mapped to its OWASP id, and confidence-tagged. Writes an approvable ledger (`docs/security-audit.md`) with an OWASP coverage table + a `Fix?` tick column and holds the **single human gate** — you tick what gets fixed. Then, per ticked finding, `security-fixer` (new agent) fixes **exploit-test-first**: a failing test that proves the hole → the minimal fix reusing repo patterns → the exploit test and narrowest suite both green; the orchestrator **re-verifies** each fix and surfaces rotation notes for secrets. `--find-only` stops at the gate for teams that fix through their own pipeline. Subagents: `scale-security-reviewer` (shared with `feature-review` — one rulebook, two scopes) + `security-fixer`. |
